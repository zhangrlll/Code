...
% maxdim=2;   % max dim(
[len,width]=size(x); 
Dimensions=zeros(1,width);   %二维表示有6个角度 
Dimensions(1,:)=bins;
hist = zeros(Dimensions);  %6*6*6*6*6*6
...
if width==7
    for i=1:len
        % change the number if max dim changes  ,Labels(i,3),Labels(i,4),Labels(i,5),Labels(i,6)
%         hist(sub2ind(size(hist),Labels(i,1),Labels(i,2),Labels(i,3),Labels(i,4),Labels(i,5),Labels(i,6)))=hist(sub2ind(size(hist),Labels(i,1),Labels(i,2),Labels(i,3),Labels(i,4),Labels(i,5),Labels(i,6)))+1;
        hist(Labels(i,1),Labels(i,2),Labels(i,3),Labels(i,4),Labels(i,5),Labels(i,6),Labels(i,7))=hist(Labels(i,1),Labels(i,2),Labels(i,3),Labels(i,4),Labels(i,5),Labels(i,6),Labels(i,7))+1;
    end
end
hist=hist+eps;
